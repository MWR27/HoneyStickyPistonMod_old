   ...
   public boolean triggerEvent(BlockState state, World worldIn, BlockPos pos, int id, int param) {
      ...
      if (id == 0) {
         ...
         if (this.isSticky) {
            ...
            if (!flag1) {
               if (id != 1 || blockstate1.isAir() || !canPush(blockstate1, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate1.getPistonPushReaction() != PushReaction.NORMAL && !blockstate1.is(Blocks.PISTON) && !blockstate1.is(Blocks.STICKY_PISTON) && !blockstate1.is(ModBlocks.HONEY_STICKY_PISTON.get())) {
   ...
   public void playerWillDestroy(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
       if (state.getValue(EXTENDED)) {
           ...
           if (direction.compareTo(Direction.UP) == 0) {
               worldIn.destroyBlock(blockPos.above(), false);
           }
           else if(direction.compareTo(Direction.DOWN) == 0) {
               worldIn.destroyBlock(blockPos.below(), false);
           }
